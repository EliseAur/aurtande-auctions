...
export async function amountWinningBids() {
  ...
  // console.log("Listings I won", profileWinsListings);
  ...
  profileWinsListings.forEach((listing) => {
    ...
    if (wins.length > 0) {
      ...
      // Log the amount of the last bid
      // console.log(
      //   `Winning bid amount for listing ${listing.id}: ${lastBidAmount}`,
      // );
    } else {
      // Log a message if there are no bids for the listing
...
export async function amountMyListings() {
  ...
  // console.log("Listings I created", profileListings);
  ...
  profileListings.forEach((listing) => {
    ...
    if (new Date(listing.endsAt) <= currentDateTime) {
      ...
      if (bids.length > 0) {
        ...
        // Log the amount of the last bid
        // console.log(
        //   `Winning amount for my finished listing ${listing.id}: ${lastBid.amount}`,
        // );
      } else {
  ...
  // Log the total amount of the last bids for finished listings
